...
def extract_features_from_files(file_paths: list) -> np.ndarray:
    """
    Extract features from multiple audio files in parallel.
    Returns a 2D array of shape (num_files, 45).
    """
    with ThreadPoolExecutor(max_workers=N_WORKERS) as executor:
        features = list(tqdm(executor.map(extract_features, file_paths), total=len(file_paths), 
...
def load_data_set() -> pd.DataFrame:
    ...
    return pd.DataFrame({"Path": file_paths, "Emotion": labels})
...
def train_model():
    ...
    # Extract features
    X = extract_features_from_files(df['Path'].tolist())
    ...
    print(classification_report(y_test, y_pred, target_names=pd.Categorical(df['Emotion']).categories))
